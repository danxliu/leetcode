...
void floodfill(vector<vector<int>> &grid, pair<int, int> start) {
    ...
    while (!q.empty()) {
        ...
        for (int i=0; i<4; i++) {
            ...
            if (next.first < 0 || next.second < 0 || next.first >= grid.size() || next.second >= grid[0].size()) continue;
...
int numEnclaves(vector<vector<int>> &grid)
{
    ...
    int m = grid[0].size();
    for (int i=0; i<n; i++) {
        if (grid[i][0] == 1) floodfill(grid, {i, 0});
        if (grid[i][m-1] == 1) floodfill(grid, {i, m-1});
    }
    for (int i=0; i<m; i++) {
        if (grid[0][i] == 1) floodfill(grid, {0, i});
        if (grid[n-1][i] == 1) floodfill(grid, {n-1, i});
    }
